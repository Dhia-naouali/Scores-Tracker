...
class Weights:
    def __init__(self, final, midterm, lab):
        ...
        if all(v > 0 for v in tests) and round(sum(tests)) == 1:
            ...
            print(tests)
            print(sum(tests))
            raise ValueError("invalid weights within subject")
...
class Subject:
    ...
    def __init__(self, name, unit, weight, weights):
        if weight <= 0:
            raise ValueError("negative subject weight")
        self.name = name
        self.unit = unit
        self.weight = weight
...
"""
pre-engineering student curriculum
"""
...
from data import get_score, MAX_TITLE_LEN, get_average
